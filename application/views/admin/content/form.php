...
<div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 mb-4">
	<div>
		...
		<p class="text-muted mb-0"><?php echo $is_view ? 'Review the current record details and access rights before making changes.' : 'Maintain high-quality, church-centered content that is clear, pastoral, and welcoming.'; ?></p>
	</div>
	<div class="d-flex gap-2">
		<a class="btn btn-outline-secondary" href="<?php echo site_url('admin/content/'.$module); ?>">Back to List</a>
		<?php if ($is_view && !empty($item->id) && (!isset($config['allow_edit']) || $config['allow_edit'] !== FALSE)): ?>
			<a class="btn btn-admin" href="<?php echo site_url('admin/content/'.$module.'/edit/'.$item->id); ?>">Edit Record</a>
		<?php endif; ?>
	...
	<div class="admin-card">
		<div class="row g-4">
			<?php foreach ($fields as $field): ?>
				...
				$stored_value = form_value($item, $field_name);
				$current_value = set_value($field_name, $stored_value);
				if ($field_type === 'checkbox_group')
				{
					$current_value = isset($_POST[$field_name]) && is_array($_POST[$field_name]) ? $_POST[$field_name] : normalize_role_permissions($stored_value, TRUE);
				...
				<div class="col-lg-<?php echo in_array($field_type, array('textarea', 'checkbox_group'), TRUE) ? '12' : '6'; ?>">
					...
					<?php if ($field_type === 'textarea'): ?>
						<textarea class="form-control" id="<?php echo html_escape($field_name); ?>" name="<?php echo html_escape($field_name); ?>" rows="6" <?php echo $is_view ? 'readonly' : ''; ?>><?php echo html_escape($current_value); ?></textarea>
					<?php elseif ($field_type === 'select'): ?>
						<select class="form-select" id="<?php echo html_escape($field_name); ?>" name="<?php echo html_escape($field_name); ?>" <?php echo $is_view ? 'disabled' : ''; ?>>
					...
					<?php elseif ($field_type === 'checkbox'): ?>
						<div class="form-check form-switch mt-2">
							<input class="form-check-input" type="checkbox" id="<?php echo html_escape($field_name); ?>" name="<?php echo html_escape($field_name); ?>" value="1" <?php echo !empty($current_value) ? 'checked' : ''; ?> <?php echo $is_view ? 'disabled' : ''; ?>>
							...
							<?php foreach ($field['options'] as $option_key => $option_label): ?>
								<div class="col-sm-6">
									<label class="form-check border rounded-4 px-3 py-2 h-100 d-flex align-items-start gap-2">
										<input class="form-check-input mt-1" type="checkbox" name="<?php echo html_escape($field_name); ?>[]" value="<?php echo html_escape($option_key); ?>" <?php echo in_array((string) $option_key, $current_value, TRUE) ? 'checked' : ''; ?> <?php echo $is_view ? 'disabled' : ''; ?>>
										<span>
					...
					<?php elseif ($field_type === 'file'): ?>
						...
						<?php elseif ($is_view): ?>
							<div class="form-text">No file uploaded.</div>
						<?php endif; ?>
					<?php else: ?>
						<input class="form-control" id="<?php echo html_escape($field_name); ?>" type="<?php echo $field_type === 'datetime' ? 'datetime-local' : html_escape($field_type); ?>" name="<?php echo html_escape($field_name); ?>" value="<?php echo html_escape($field_type === 'datetime' && !empty($current_value) ? date('Y-m-d\TH:i', strtotime($current_value)) : $current_value); ?>" <?php echo $is_view ? 'readonly' : ''; ?>>
					<?php endif; ?>
		...
		<div class="d-flex flex-wrap gap-2 mt-4">
			...
			<?php else: ?>
				<?php if (!empty($item->id) && (!isset($config['allow_edit']) || $config['allow_edit'] !== FALSE)): ?>
					<a class="btn btn-admin" href="<?php echo site_url('admin/content/'.$module.'/edit/'.$item->id); ?>">Edit Record</a>
				<?php endif; ?>
				<a class="btn btn-outline-secondary" href="<?php echo site_url('admin/content/'.$module); ?>">Back to List</a>
			<?php endif; ?>
